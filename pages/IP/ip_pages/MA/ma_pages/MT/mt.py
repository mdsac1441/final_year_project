...
def write():
    ...
    def load_data():
        ...
        for c in col_names:
            data[c] = data[c].replace("?", numpy.NaN)
    ...
    df = load_data()
    class_names = ['More than 50k', 'Less than 50k']
    ...
    if classifier == 'XGBClassifier':
        ...
        learning_rate = st.sidebar.number_input("learning_rate", 0.05, 0.3, step=.05,
                                                key='learning_rate')
        min_child_weigh = st.sidebar.number_input(
            "min_child_weigh", 1, 7, step=1)

        gamma = st.sidebar.number_input("gamma", 0.0, 0.4, step=0.1,
                                        key='gamma')
        colsample_bytree = st.sidebar.number_input(
            "colsample_bytree", 0.3, 0.7, step=0.1, key="colsample_bytree")

        n_estimators = st.sidebar.number_input(
            "n_estimators", 100, 5000, step=10, key="n_estimators")
    ...
    if classifier == 'GradientBoostingClassifier':
        ...
        n_estimators = st.sidebar.number_input("n_estimators", 100, 5000, step=10,
                                               key='n_estimators')
        max_depth = st.sidebar.number_input(
            "The maximum depth of the tree", 3, 15, step=1, key='max_depths')
